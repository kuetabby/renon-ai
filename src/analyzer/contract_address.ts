...
<b>Did you choose the right network? You scanned this contract on ${info.code}</b>
      ...
      const overall = [
        Number(isAntiWhale),
        Number(!isSlippageModifiable),
        Number(!isHiddenOwner),
        Number(!isTradingCooldown),
        Number(!isBlacklist),
        Number(!isWhitelist),
        Number(!isProxy),
        Number(!isMintable),
        Number(!isTransferPausable),
        Number(!isTakeBackOwnership),
      ].reduce((prev, acc) => {
        return prev + acc;
      }, 0);
...
<b>🍯 Honeypot: ${Number(is_honeypot) === 0 ? '✅' : '❌'}</b>
...
<pre>🚨 Quick Score is ${overall * 10} ${scoreIcon(overall)}
⚙️ Overview Result is ${scoreText(overall)}

<b>${isAntiWhale ? '🟢' : '🔴'} Anti Whale</b>
<b>${!isSlippageModifiable ? '🟢' : '🔴'} Update Fees Function</b>
<b>${!isHiddenOwner ? '🟢' : '🔴'} Hidden Owner</b>
<b>${!isTradingCooldown ? '🟢' : '🔴'} Cooldown Function</b>
<b>${!isBlacklist ? '🟢' : '🔴'} Blacklist Function</b>
<b>${!isWhitelist ? '🟢' : '🔴'} Whitelist Function</b>
<b>${!isProxy ? '🟢' : '🔴'} Proxy Contract</b>
<b>${!isMintable ? '🟢' : '🔴'} Minting Function</b>
<b>${!isTransferPausable ? '🟢' : '🔴'} Pausable Contract</b>
<b>${!isTakeBackOwnership ? '🟢' : '🔴'} Retrieve Ownership Function</b>
</pre>
<i>Note: These tools are not intended as financial advice.</i>
        `,
        {
          parse_mode: 'HTML',
          // chat_id: chatId,
